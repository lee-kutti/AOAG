...
class android_heap():
    ...
    def getTLAB(self, t, memList):
        ...
        with open(tpath, 'rb') as g:
            ...
            tidIndex = types.art_types.get('struct_tlsPtr_')[1].get(
                'thread_local_start')[0] + tlsIndex
            ...
            # TLAB_lmt = hex(unpack_int(g.read(4))[0]) # khong can dung den
    ...
    def getBitmap(self, regionSPath, offset, memList):
        mark_bitmap = self.readPointer(
            regionSPath, offset,
            164)  #GetLiveBitmap for region space returns mark_bitmap
        ...
        g.seek(offset)
        # memmap = hex(unpack_int(g.read(4))[0]) # khong can dung den
        # begin_ = hex(unpack_int(g.read(4))[0]) # khong can dung den
        bitmap_size_ = unpack_dec(g.read(4))[0]
        #print "bitmap_size " + str(bitmap_size_)
        #print "Bitmap size = "+ str(bitmap_size_)
        heapBegin_ = hex(unpack_int(g.read(4))[0])
        #print "heapBegin " + str(heapBegin_)
    ...
    def getRegion(self, nPath, rAddr, memList):
        ...
        # regionSpace = self.readPointer(heapPath, offset, 464)
        ...
        # num_non_free_regions_ = self.readInt(regionSPath, offset, 104) # khong can dung den
        ...
        [bitmap_size_, heapBegin_] = self.getBitmap(regionSPath, offset,
                                                    memList)
...
#To write code that checks GetLiveBitmap()->VisitMarkedRange(pos, top, ...) Line253 regions_space-inl.h
    ...
    def regionHdr(self, regionAddr, num_regions_, memList):
        ...
        while (count < num_regions_):
            ...
            if (
                    state_ > 0
            ):  # non-free region state_=1 means kRegionStateAllocated, our none-free regions are type = 3 (kRegionTypeToSpace), 4 (kRegionTypeNone)
                g.seek(regBegin + 33)
                ...
                if (is_a_tlab_ == True):
                    ...
                    TLAB.append(
                        str(idx) + "\t" + begin_ + "\t" + top_ + "\t" + end_ +
                        "\t" + str(need_bitmap) + "(" + str(liveBytes) + " " +
                        str(diff) + ")" + "\t\t\t" + thread)
                else:
                    g.seek(regBegin)
                    idx = unpack_dec(g.read(4))[0]
                    ...
                    NonTLAB.append(
                        str(idx) + "\t" + begin_ + "\t" + top_ + "\t" + end_ +
                        "\t" + str(objAlloc) + "\t" + str(need_bitmap) + "(" +
                        str(liveBytes) + " " + str(diff) + ")")
                    g.seek(regBegin + 40)
            else:
                g.seek(regBegin + 40)
            count = count + 1
    ...
    def get_open_fds(self):
        pid = os.getpid()
        procs = subprocess.check_output(["lsof", '-w', '-Ff', "-p",
                                         str(pid)]).split('\n')
        print len(procs)
