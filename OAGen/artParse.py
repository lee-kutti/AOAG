...
def getAddrRange(lstList):
    for entry in lstList:
        listing.update(
            {entry.split()[1]: [j for j in entry.split() if ("0x") in j]})
    return listing
...
def getLibART(bss, instance):  #search for libart.so
    ...
    if ("[anon:.bss]") in bss:
        ...
        #print instance
        #print start
        insAddr = int("0x" + instance[len(instance) - 4:], 16) - int(
            "0x" + start[len(start) - 4:], 16)
    return insAddr
...
def getRuntime(path):  #Get runtime instance
    libart = [
        ...
        if filename.startswith("libart.so")
    ][0]
    process = subprocess.check_output("nm -aS " + path + "/" + libart +
                                      " | grep \"_ZN3art7Runtime9instance_E\"",
                                      shell=True)
    return process.split()[0]


def getBss(lstList, path,
           instance):  #get bss section and search for runtime instance
    libRange = [i for i in lstList if ("/libart.so") in i
                ]  #find all insances of libart in mfetch.lst
    address = [j for j in libRange[0].split()
               if ("0x") in j]  #find begin and end address
...
def getBootART(lstList, path,
               fname):  #get bss section and search for runtime instance
    libRange = [i for i in lstList
                if (fname) in i]  #find all insances of libart in mfetch.lst
    address = [j for j in libRange[0].split()
               if (".bin:") in j]  #find begin and end address
...
def main(projPath):
    ...
    lst = [
        filename for filename in os.listdir(path) if filename.endswith("lst")
    ][0]
    lstFile = path + "/" + lst
    if lst == "mfetch.lst":  # Its mefetch dump
        ...
        [listing, lstList
         ] = parseVolFile(lstFile)  # Its linux_dump_map dump from volatility
    [address] = getBss(lstList, path, instance)
    [
        memList.update({key: value}) for key, value in listing.items()
        if key.startswith("mem")
    ]
    [
        mapList.update({key: value}) for key, value in listing.items()
...
def helper(hp, th, nPath, rAddr, path, memList):
    [regionAddr, num_regions_, bitmap_size_,
     heapBegin_] = hp.getRegion(nPath, rAddr, memList)
