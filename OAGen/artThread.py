...
class android_threads():
    ...
    def getThreads(self, tList, memList):
        ...
        #print "tls32Index " + str(tls32Index)

        tidIndex = types.art_types.get('struct_tls32_')[1].get(
            'tid')[0] + tls32Index
        #print "tidIndex " + str(tidIndex)

        tlsPtrIndex = types.art_types.get('Thread')[1].get('tlsPtr_')[0]
        #print "tlsPtrIndex " + str(tlsPtrIndex)

        nameIndex = tlsPtrIndex + types.art_types.get('struct_tlsPtr_')[1].get(
            'name')[0]
        #print "nameIndex " + str(nameIndex)

        sIndex = tlsPtrIndex + types.art_types.get('struct_tlsPtr_')[1].get(
            'opeer')[0]
        ...
        for t in tList:
            ...
            with open(tpath, 'rb') as g:
                ...
                #print "first seek " + str(offset + tidIndex)
                g.seek(offset + tidIndex)

                tid = unpack_dec(g.read(4))[0]

                #print "second seek " + str (offset+nameIndex)
                g.seek(offset + nameIndex)
                strPointer = hex(unpack_int(g.read(4))[0])

                #print ("third seek ") + str (offset + sIndex)
