...
unpack_b = lambda fhandle: struct.unpack('<B', fhandle.read(1))[
    0]  #Byte or Bool
...
def getAddrRange(lstList):
    for entry in lstList:
        listing.update({
            entry.split()[1][:-1]:
            re.search('[0-9a-f]{8,16}-[0-9a-f]{8,16}',
                      entry).group(0).split('-')
        })
    return listing
...
def getRuntime(path):  #Get runtime instance
    #tmp = [filename for filename in os.listdir(path) if filename.startswith("libart.so")]
    libart = [
...
def getBss(lstList, path,
    ...
    process = subprocess.check_output(
        ...
        '/libart.so | grep "LOAD" -A1 | grep "R E" -B1',
        shell=True)
