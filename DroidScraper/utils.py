...
unpack_addr = lambda fhandle: unpack_uint(
    fhandle) if ARCH == 32 else unpack_ulong(fhandle)
...
if ARCH == 32:
    if VERSION == '8.0':
        ...
        raise ImportError(
            'Offsets cannot be imported. Supported versions are: 8.0, 8.1 and 9'
        )
elif ARCH == 64:
    if VERSION == '8.0':
    ...
    raise ImportError(
        'Offsets cannot be imported. Supported architectures are: 32 and 64')

get_index = lambda class_name, attribute_name: types.get(class_name)[1].get(
    attribute_name)[0]
